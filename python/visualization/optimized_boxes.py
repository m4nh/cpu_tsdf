...
class Instance(object):
    ...
    def fromFile(filename):
        ...
        print "LOading", name, label
        data = np.loadtxt(filename)
        return Instance(points=data, label=label)
...
for instance_folder in folders:
    ...
    for f in files:
        if 'boxes' in f:
            ...
            instances.append(Instance.fromFile(f))

    out_instances = []
    for inst in instances:
        print "Instance", inst.label
        print inst.shortRepresentation()
        inst.optimizeSVD()
        print inst.shortRepresentation()
        out_instances.append(inst.shortRepresentation())
    out_file = os.path.join(instance_folder, "boxes_optimized.txt")
    np.savetxt(out_file, np.array(out_instances))
